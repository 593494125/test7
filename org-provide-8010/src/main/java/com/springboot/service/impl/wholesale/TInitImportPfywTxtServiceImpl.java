...
import com.springboot.dao.wholesale.TInitImportPfywTxtMapper;
...
@Slf4j
public class TInitImportPfywTxtServiceImpl extends ServiceImpl<TInitImportPfywTxtMapper, TInitImportPfywTxt> implements TInitImportPfywTxtService {

    @Resource
    private TInitImportPfywTxtMapper tInitImportPfywTxtMapper;
    @Resource
    ...
    @Override
    public boolean saveBatch(String sixCode, List<TInitImportPfywTxt> list) {
        //接收集合各段的 执行的返回结果
...
//        int sunSum = 3;
        ...
        try {
            ...
            for (int i = 0; i < sunSum; i++) {
                ...
                //线程切断**/
                List<TInitImportPfywTxt> sunList = list.subList(listStart,listEnd);
...
//            taskExecutor.shutdown();
    ...
    public boolean threadPoolSaveData(String sixCode,List<TInitImportPfywTxt> sunList){
        ...
        try {
            ...
            for (TInitImportPfywTxt bean : sunList) {
                ...
                ps.setString(58, bean.getBz4());
                ps.setString(59, bean.getTmbh());
                ps.setInt(60, bean.getSl());
                ps.setString(61, bean.getTmfa());
                ps.setString(62, bean.getEpcNo());
                ps.setString(63, bean.getSpbh());
                ps.addBatch();
            }
            // 执行批量入库
            ps.executeBatch();
    ...
    public boolean deleteOldData(String sixCode, String yhbh, String ywflag) {
        boolean deleteFlag=tInitImportPfywTxtMapper.deleteOldData(yhbh,ywflag);
        return deleteFlag;
    }
    class ImportTask implements Runnable {
        ...
        public void run() {
            if (null != list) {
                tInitImportPfywTxtService.threadPoolSaveData(sixCode, list);
                log.info("thread id===五位码===="+sixCode);

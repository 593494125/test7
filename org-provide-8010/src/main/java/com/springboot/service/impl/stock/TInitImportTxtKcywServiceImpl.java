...
import com.springboot.dao.stock.TInitImportTxtKcywMapper;
...
@Slf4j
public class TInitImportTxtKcywServiceImpl extends ServiceImpl<TInitImportTxtKcywMapper, TInitImportTxtKcyw> implements TInitImportTxtKcywService {

    @Resource
    private TInitImportTxtKcywMapper tInitImportTxtKcywMapper;
    @Resource
    ...
    @Override
    public boolean saveBatch(String sixCode, List<TInitImportTxtKcyw> list) {
        //接收集合各段的 执行的返回结果
        //集合总条数
        int size = list.size();
        //将集合切分的段数(2*CPU的核心数)
...
//        int sunSum = 3;
        ...
        try {
            ...
            for (int i = 0; i < sunSum; i++) {
                ...
                //线程切断**/
                List<TInitImportTxtKcyw> sunList = list.subList(listStart,listEnd);
...
//            taskExecutor.shutdown();
    ...
    public boolean threadPoolSaveData(String sixCode,List<TInitImportTxtKcyw> sunList){
        ...
        try {
            ...
            for (TInitImportTxtKcyw bean : sunList) {
    ...
    public boolean deleteOldData(String sixCode, String yhbh, String ywflag) {
        boolean deleteFlag=tInitImportTxtKcywMapper.deleteOldData(yhbh,ywflag);
        return deleteFlag;
    }
    class ImportTask implements Runnable {
        ...
        public void run() {
            if (null != list) {
                tInitImportTxtKcywService.threadPoolSaveData(sixCode, list);
            }
            // 发出线程任务完成的信号

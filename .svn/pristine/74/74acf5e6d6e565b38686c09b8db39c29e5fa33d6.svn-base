...
import com.springboot.dao.transfer.TInitImportDbywTxtMapper;
...
@Slf4j
public class TInitImportDbywTxtServiceImpl extends ServiceImpl<TInitImportDbywTxtMapper, TInitImportDbywTxt> implements TInitImportDbywTxtService {

    @Resource
    private TInitImportDbywTxtMapper tInitImportDbywTxtMapper;
    @Resource
    ...
    @Override
    public boolean saveBatch(String sixCode, List<TInitImportDbywTxt> list) {
        //接收集合各段的 执行的返回结果
        //集合总条数
        int size = list.size();
        //将集合切分的段数(2*CPU的核心数)
...
//        int sunSum = 3;
        ...
        CountDownLatch countDownLatch = new CountDownLatch(sunSum);
            for (int i = 0; i < sunSum; i++) {
                ...
                //线程切断**/
                List<TInitImportDbywTxt> sunList = list.subList(listStart,listEnd);
...
//            taskExecutor.shutdown();
    ...
    public boolean threadPoolSaveData(String sixCode,List<TInitImportDbywTxt> sunList){
        ...
        try {
            ...
            for (TInitImportDbywTxt bean : sunList) {
    ...
    public boolean deleteOldData(String sixCode, String yhbh, String ywflag) {
        boolean deleteFlag=tInitImportDbywTxtMapper.deleteOldData(yhbh,ywflag);
        return deleteFlag;
    }
    class ImportTask implements Runnable {
        ...
        public void run() {
            if (null != list) {
                tInitImportDbywTxtService.threadPoolSaveData(sixCode, list);
            }
            // 发出线程任务完成的信号

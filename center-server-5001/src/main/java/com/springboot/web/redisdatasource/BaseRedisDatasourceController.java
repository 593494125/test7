...
//        String sixCode=redisUtil.get(SystemConstant.DEFAULT_REDIS_KEY,baseRedisDatasource.getOrgId());
        ...
        if(baseAuth!=null){
            ...
            if(StringUtils.isNotEmpty(sixCode)){
                ...
                //获取组织org对象
                String orgId=baseAuth.getOrgId();
                BaseOrg org=baseOrgService.findById("",orgId);
                if(org!=null){
                    String areaServiceUrl=org.getAreaServiceUrl();
                    //删除各个应用上redis数据源连接
                    String result1=CommonGetRedisValue.updateRedisDataSource(sixCode,baseRedisDatasource,areaServiceUrl);
                    ...
                    return  ResultGenerator.genErrorSetMsgErrorResult("该企业为空,数据库中无数据!");
                }
            }else{
     ...
     * redis区域服务器下拉框接口
     */
    @PostMapping("/getRedisSelectList")
    public Result getRedisSelectList(@RequestBody String jsons){
        Result result=null;
        //获取下拉框
        ...
        if(jsonObject!=null){
            ...
            if(StringUtils.isNotEmpty(type)&&"2".equals(type)){
                if(StringUtils.isNotEmpty(areaAddress)){
                    //通过redis区域地址获取区域服务器地址
                    List<String> list=baseRedisDatasourceService.getRedisSelectList("",areaAddress);
                    result=ResultGenerator.genSuccessResult(list);
                }else{
                    result=ResultGenerator.genErrorSetMsgErrorResult("区域地址为空,请先选择区域地址!");
                }
            }else{
                List<String> list=baseRedisDatasourceService.getAreaSelectList("");
                result=ResultGenerator.genSuccessResult(list);
            }
        }
        return  result;

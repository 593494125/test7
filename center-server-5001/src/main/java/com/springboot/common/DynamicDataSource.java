...
//            HikariDataSource druidDataSource = new HikariDataSource();
            ...
            druidDataSource.setInitialSize(20); //初始化时建立物理连接的个数。初始化发生在显示调用init方法，或者第一次getConnection时
            druidDataSource.setMaxActive(200); //最大连接池数量
            druidDataSource.setMinIdle(20); //最小连接池数量
            ...
            druidDataSource.setMaxEvictableIdleTimeMillis(250000);
            ...
            druidDataSource.setRemoveAbandonedTimeout(1800); //泄露连接的定义时间(要超过最大事务的处理时间)；单位为秒。这里配置为1小时
...
//            log.info(key+"数据源初始化成功");
    ...
    public boolean createDataSourceWithCheck(BaseDatasource dataSource) throws Exception {
        boolean flag1=true;
...
//                log.info("数据源"+datasourceId+"正常");
            ...
            if(rightFlag) {
                ...
                log.info("准备重新创建数据源...");
                flag1= createDataSource(dataSource);
                log.info("重新创建数据源完成");
            }
        } else {
            flag1=createDataSource(dataSource);
    ...
    public  boolean createDataSource(BaseDatasource dataSource) throws Exception {
        String datasourceId = dataSource.getId();

...
import com.springboot.dao.stock.YwKcKctzdHzMapper;
...
@Slf4j
public class YwKcKctzdHzServiceImpl extends ServiceImpl<YwKcKctzdHzMapper, YwKcKctzdHz> implements YwKcKctzdHzService {

    @Autowired
    private RedisClient redisClient;
    @Resource
    private YwKcKctzdHzMapper ywKcKctzdHzMapper;
    @Resource
    ...
    @Resource
    private YwKcDjztService ywKcDjztService;
    @Autowired
    private TImportErrorListService tImportErrorListService;

    @Override
    public Page<YwKcKctzdHz> getPage(String sixCode, Page<YwKcKctzdHz> page, YwKcKctzdHz ywKcKctzdHz, Map<String, String> param) {
        //获取当天日期
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS");
        Integer pageNo=ywKcKctzdHz.getPageNo();
        Integer pageSize=ywKcKctzdHz.getPageSize();
        String yhbh=param.get("yhbh");
        List<YwKcKctzdHz> list=ywKcKctzdHzMapper.getList(SystemConstant.XXLY,yhbh,"",pageNo,pageSize);
        List<YwKcKctzdHz> newList=this.changeList(sixCode,yhbh,list);
    ...
    public List<YwKcKctzdHz> changeList(String sixCode, String yhbh, List<YwKcKctzdHz> list){
        //获取当天日期
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS");
        SimpleDateFormat dateFormat1 = new SimpleDateFormat("yyyy-MM-dd");
        List<YwKcKctzdHz> newList=new ArrayList<YwKcKctzdHz>();
    ...
    @Override
    public Page<YwKcKctzdHz> getQueryPage(String sixCode, Page<YwKcKctzdHz> page, YwKcKctzdHzQueryJson ywKcKctzdHzQueryJson, Map<String, String> param) {
...
//        String djlx=ywKcKctzdHzQueryJson.getDjlx();
        List<YwKcKctzdHz> list=ywKcKctzdHzMapper.getQueryPage(SystemConstant.XXLY,yhbh,ywKcKctzdHzQueryJson.getStartTime(),ywKcKctzdHzQueryJson.getEndTime(),
    ...
    @Override
    public AddReturnJson saveBean(String sixCode, YwKcKctzdHz ywKcKctzdHz) {
        ...
        //先删除原来临时数据
        deleteFlag= tInitImportTxtKcywService.deleteOldData(sixCode,ywKcKctzdHz.getCzy(),SystemConstant.KT_TYPE);
        log.info("删除采购临时表状态："+deleteFlag);
        long endd = System.currentTimeMillis();
        log.info("删除采购临时表状态，耗时：" + (endd - start) + "毫秒");
        String tgflag=ywKcKctzdHz.getCzy()+ JBDate.getNowDate(new Date().getTime());
        //临时表中批量插入数据
        String[] tmbhAndslArray=ywKcKctzdHz.getTmbhAndsl();
        if(tmbhAndslArray!=null&&tmbhAndslArray.length>0){
            ...
            for (int i = 0; i < k; i++) {
                ...
                if(array!=null&&array.length>0){
                    ...
                    getTInitImportKcywList(tmbh, sl, j, tgflag, ywKcKctzdHz, list, tInitImportKcyw1, dateFormat, bean);
            ...
            if(list!=null&&list.size()>0){
                ...
                if(flag==true){
                    //调用存储过程增加明细和汇总等其他表数据
                    String msg= ywCgDdmxService.initCgDdmx(sixCode,SystemConstant.KT_TYPE,tgflag);
                    ...
                    if(StringUtils.isNotEmpty(msg)&&msg.startsWith("OK")){
                        ...
                        if(StringUtils.isNotEmpty(pzh)){
                            YwKcKctzdHz queryYwKcKctzdHz=new YwKcKctzdHz();
                            queryYwKcKctzdHz.setXxly(SystemConstant.XXLY);
                            queryYwKcKctzdHz.setBz(ywKcKctzdHz.getBz());
                            Integer count=ywKcKctzdHzMapper.update(queryYwKcKctzdHz,new EntityWrapper<YwKcKctzdHz>().eq("pzh",pzh));
    ...
    public void getTInitImportKcywList(String tmbh,String sl,int j,String tgflag,YwKcKctzdHz ywKcKctzdHz,List<TInitImportTxtKcyw> list, TInitImportTxtKcyw tInitImportKcyw1, SimpleDateFormat dateFormat, DaSpSpDaJson bean){
        ...
        if(bean!=null){
            ...
            tInitImportKcyw.setGysbh(ywKcKctzdHz.getSgdj()+"#"+ywKcKctzdHz.getCzy());
            tInitImportKcyw.setYhbh(ywKcKctzdHz.getCzy());

            tInitImportKcyw.setYwflag(SystemConstant.KT_TYPE);
            tInitImportKcyw.setBz3("phtz");

            tInitImportKcyw.setBz4("");
            tInitImportKcyw.setBz1(ywKcKctzdHz.getTzbm());
            tInitImportKcyw.setBz2(dateFormat.format(ywKcKctzdHz.getTzrq()));
            ...
            tInitImportKcyw.setBzxx(ywKcKctzdHz.getBz());
    ...
    public Integer deleteByPzh(String sixCode, String pzh) {
        ...
        Integer flag2=0;
        //删除采购入库单汇总同时删除关联表
        flag=ywKcKctzdHzMapper.delete(pzh);
        log.info("删除汇总："+flag);
        //批量删除采购入库单明细
        if(flag>0) {
            flag1 = ywKcKctzdMxService.batchDeleteByPzh(sixCode, pzh);
            log.info("删除明细：" + flag1);
            //删除采购入库单状态表
            flag2 = ywKcDjztService.batchDeleteByPzh(sixCode, pzh);
            log.info("删除状态：" + flag2);
        }
        return flag;
    }
    ...
    public List<String> getPzh(String sixCode, Map<String, String> param) {
        ...
        List<String> list=ywKcKctzdHzMapper.getPzh(SystemConstant.XXLY,yhbh,djlx);
        return list;
    }

    @Override
    public YwKcKctzdHz findByPzh(String sixCode, String pzh,String sgdj) {
        YwKcKctzdHz bean =ywKcKctzdHzMapper.findByPzh(pzh,sgdj);
        return bean;
    }
